...
import com.mstudio.cryptonews.data.entities.SearchFeedResult
...
class FeedSearchDialog(context: Context, val search: String = "", private var defaultFeeds: List<SearchFeedResult> =/* generateDefaultFeeds(context)*/ ArrayList())
    : SimpleSearchDialogCompat<SearchFeedResult>(context,
        ...
        SearchResultListener<SearchFeedResult> { dialog, item, position ->
            ...
            val feedToAdd = Feed(link = item.link, title = item.name, retrieveFullText = true)
//            feedToAdd.retrieveFullText = defaultFeeds.contains(item) // do that automatically for google news feeds

            context.doAsync { com.mstudio.cryptonews.App.db.feedDao().insert(feedToAdd) }
        }), AnkoLogger {
