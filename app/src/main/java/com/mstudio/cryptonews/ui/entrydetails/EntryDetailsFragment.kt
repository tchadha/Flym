...
import com.mstudio.cryptonews.utils.isOnline
...
class EntryDetailsFragment : Fragment() {
	...
	private fun initDataObservers() {
		...
		isMobilizingLiveData = com.mstudio.cryptonews.App.db.taskDao().observeItemMobilizationTasksCount(entryId)
		isMobilizingLiveData?.observe(this, Observer<Int> { count ->
			if (count ?: 0 > 0) {
				...
				if (isMobilizing) {
					doAsync {
						com.mstudio.cryptonews.App.db.entryDao().findByIdWithFeed(entryId)?.let { newEntry ->
							uiThread {
				...
				setOnMenuItemClickListener { item ->
					when (item?.itemId) {
						R.id.menu_entry_details__favorite -> {
							...
							doAsync {
								com.mstudio.cryptonews.App.db.entryDao().update(entryWithFeed.entry)
							}
						}
						...
						R.id.menu_entry_details__mark_as_unread -> {
							doAsync {
								com.mstudio.cryptonews.App.db.entryDao().markAsUnread(listOf(entryId))
							}
							if (activity?.containers_layout?.hasTwoColumns() != true) {
...
//		}
	...
	fun setEntry(entryId: String, allEntryIds: List<String>) {
		...
		doAsync {
			com.mstudio.cryptonews.App.db.entryDao().findByIdWithFeed(entryId)?.let { entry ->
				entryWithFeed = entry
				preferFullText = true
				isMobilizing = false
